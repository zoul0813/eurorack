...
void updateControl()
{
  int val_knob1 = mozziAnalogRead(KNOB_1);
  int val_knob2 = mozziAnalogRead(KNOB_2);
  int val_knob3 = mozziAnalogRead(KNOB_3);
  int val_cv1 = mozziAnalogRead(CV_1);
  int val_cv2 = mozziAnalogRead(CV_2);
  int val_voct = mozziAnalogRead(VOCT);

  // chord setting
  chord = (val_knob3 / 128) + (val_cv2 / 128);
  chord = constrain(chord, 0, 7);

  // inversion setting
  inv_knob = val_knob2;
  inv = (inv_knob / 64) + (val_cv1 / 64);
  inv = constrain(inv, 0, 15);

  if (inv_knob < 1020)
  { // when selecting wave , not apply
    switch (inv)
    {
    case 0:
      inv_aply1 = 0;
  ...
  Serial.print(val_knob1);
  Serial.print(", ");
  Serial.print(val_knob2);
  Serial.print(", ");
  Serial.print(val_knob3);
  Serial.print(", ");
  Serial.print(val_cv1);
  Serial.print(", ");
  Serial.print(val_cv2);
  Serial.print(", ");
  Serial.print(val_voct);
  Serial.println("");
}
